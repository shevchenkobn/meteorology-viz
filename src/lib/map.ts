...
export class GuardedMap<K, V> extends Map<K, NonOptional<V>> implements ReadonlyGuardedMap<K, V>, IGuardedMap<K, V> {
  constructor();
  constructor(entries: Maybe<Iterator<[K, NonOptional<V>]> | Iterable<[K, NonOptional<V>]>>, filterUndefined?: boolean);
  constructor(entries?: Maybe<Iterator<[K, NonOptional<V>]> | Iterable<[K, NonOptional<V>]>>, filterUndefined = false) {
    super(entries !== undefined && entries !== null ? (toGuardedMapIterable(entries, filterUndefined) as any) : null);
  }
